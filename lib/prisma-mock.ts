...
const mockPrisma = {
  ...
  user: {
    findMany: async (query?: any) => {
      if (query?.where?.role) {
        return mockUsers.filter(u => u.role === query.where.role);
      }
      return mockUsers;
    },
    findUnique: async (query: any) => mockUsers.find(u => u.email === query.where.email) || null,
    create: async (data: any) => {
    ...
    update: async (data: any) => {
      const index = mockUsers.findIndex(u => u.id === data.where.id);
